...
function editField(fieldId: string) {
    const currentValue = (document.getElementById(fieldId) as HTMLSpanElement).innerText;
    const newValue = prompt(`Введите новое значение для ${fieldId}:`, currentValue);
    if (newValue !== null) {
        (document.getElementById(fieldId) as HTMLSpanElement).innerText = newValue;
    }
}

function uploadFile(fieldId: string) {
    ...
    input.onchange = (e: any) => {
        ...
        if (file) {
            ...
            const link = document.getElementById(fieldId) as HTMLAnchorElement;
            link.href = url;
